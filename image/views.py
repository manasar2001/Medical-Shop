...
def Add_Product(request):
    ...
    if request.method=="POST":
        ...
        i = request.FILES['img']
        a = request.POST['availability']
        av = request.POST['avail']
        d = request.POST['desc']
        ct = Category.objects.get(name=c)
        Product.objects.create(category=ct, name=p, price=pr, image=i, desc=d, availability=a, avail=av)
        error=True
...
def thanks(request):
    ...
    error = ""
    try:
        sign = Profile.objects.get(user=user)
        if sign:
            error = "pat"
    except:
        pass
    d = {'error':error,'count':count}
    return render(request,'thanks.html',d)
...
def Add_supplier(request):
    ...
    if request.method=="POST":
       # c = request.POST['cat']
        ...
        #pr = request.POST['price']
        prn = request.POST['prnname']
        scon = request.POST['scontact']
        d = request.POST['date']
        ad = request.POST['add']
        #i = request.FILES['img']
        #a = request.POST['availability']
        ...
        #d = request.POST['desc']
        #ct = Category.objects.get(name=c)
        supplier.objects.create(cname=cn,sname=sn, prnname=prn,scontact=scon , date=d, add=ad)
        error=True
    d = {'error':error}
...
def Admin_View_supplier(request):
    ...
    d = {'sup':sup}
    return render(request,'admin_view_supplier.html',d)

def delete_supplier(request,pid):
    if not request.user.is_authenticated:
        return redirect('login_admin')
    sup = supplier.objects.get(id=pid)
    sup.delete()
    return redirect('admin_view_supplier')
